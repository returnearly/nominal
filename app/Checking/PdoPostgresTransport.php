...
final class PdoPostgresTransport implements PostgresTransport
{
    ...
    private function pdo(DatabaseUrl $url, int $timeoutSeconds, bool $verifyTls): PDO
    {
        if (! in_array('pgsql', PDO::getAvailableDrivers(), true)) {
            throw new RuntimeException('The PDO PostgreSQL driver is not installed.');
        }

        $database = $url->database ?? 'postgres';
        $dsn = "pgsql:host={$url->host};port={$url->port};dbname={$database};sslmode={$url->sslMode($verifyTls)}";
